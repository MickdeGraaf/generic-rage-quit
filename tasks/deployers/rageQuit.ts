...
task("deploy:rageQuit")
  ...
  .setAction(async (taskArguments: TaskArguments, { ethers }) => {
    ...
    const rageQuit: RageQuit = await rageQuitFactory.deploy(
      taskArguments.rageQuitToken,
      taskArguments.vault,
      taskArguments.vault,
    );
    ...
    console.log(
      `To verify run: npx hardhat verify ${rageQuit.address} ${taskArguments.rageQuitToken} ${taskArguments.vault} --network [NETWORK_NAME]`,
    );
  });
