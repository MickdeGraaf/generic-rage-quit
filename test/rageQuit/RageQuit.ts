...
describe("RageQuit", function () {
  ...
  it("Rage quitting ignoring some tokens should work", async () => {
    ...
    await rageQuit.rageQuit(
      INITIAL_RQT_SUPPLY,
      vaultTokenAddresses.filter(value => value != ignoredToken.address),
    );
  ...
  it("Trying to rage quit a token twice should fail", async () => {
    await expect(
      rageQuit.rageQuit(INITIAL_RQT_SUPPLY, [
        ...vaultTokenAddresses,
        vaultTokenAddresses[vaultTokenAddresses.length - 1],
      ]),
    ).to.be.revertedWith("RageQuit.rageQuit: Tokens out of order");
  });

  it("Rage quiting more than the address balance should fail", async () => {
    await expect(rageQuit.rageQuit(INITIAL_RQT_SUPPLY.add(1), vaultTokenAddresses)).to.be.revertedWith(
  ...
  it("Rage quitting while the sender did not approve the rage quit token to be spend by the contract should fail", async () => {
    ...
    await expect(rageQuit.rageQuit(INITIAL_RQT_SUPPLY, vaultTokenAddresses)).to.be.revertedWith(
